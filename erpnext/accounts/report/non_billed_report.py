...
def get_ordered_to_be_billed_data(args):
	...
	if('additional_where' in args):
		additional_where = """ and `{parent_tab}`.name not in (select `{parent_tab}`.return_against from `{parent_tab}` where `{parent_tab}`.is_return=1 )""".format(parent_tab = 'tab' + doctype)
	else:
		additional_where = ""
	# <<< Vamsi; 13012018; Excluding return items in 'Delivered Items To Be Billed' report
	...
	return frappe.db.sql("""
		...
		""".format(parent_tab = 'tab' + doctype, child_tab = 'tab' + child_tab, precision= precision, party = party,
			date_field = args.get('date'), project_field = project_field, order= args.get('order'), order_by = args.get('order_by'), uyn_custom_condition = additional_where))
